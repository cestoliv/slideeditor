...
it("seeds usage history from existing project use", () => {
  ...
  expect(userVersion(db), "the upgrade runs on open").toBe(9);
...
it("gives the font table a nullable italic media column", () => {
  const db = openDb(join(dir, "test.db"));
  const columns = db.prepare("PRAGMA table_info(font)").all() as {
    name: string;
    notnull: number;
  }[];
  const italic = columns.find((column) => column.name === "italic_media_id");
  expect(italic).toBeDefined();
  expect(italic?.notnull).toBe(0);
  db.close();
});
